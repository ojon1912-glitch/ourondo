// GAME3_3 단계: 질문 답변 (Q7, Q8)
(function() {
  var QUESTIONS = {
    7: "연애할 때 나만의 사랑 표현 방식은?",
    8: "내가 생각하는 인생의 가치관 한 줄은?"
  };
  var _userDone = false;

  window.render_GAME3_3 = function(el, room, myMember, members, isAdmin, headers, roomId, stepChanged) {
  ...
  async function renderAdminView(el, room, members, headers, roomId, esc) {
    try {
      var res = await fetch("/api/gauge/room/" + roomId + "/answers?question_nos=7,8", { headers: headers });
      ...
      var rows = members.map(function(m) {
        ...
        var answerHtml = '';
        if (isDone) {
          answers.filter(function(a) { return a.member_seq === m.member_seq && (a.question_no === 7 || a.question_no === 8); }).forEach(function(a) {
      ...
      el.innerHTML = '<div class="fade-in"><div class="text-center mb-4"><div class="text-2xl font-bold">Game 3-3 답변 현황</div><p class="text-gray-400 text-sm">' + submitted.length + '/' + members.length + '명 제출</p></div>' + rows + '<div class="text-center mt-4"><p class="text-sm text-gray-500">' + (allDone ? '모든 참여자가 답변을 제출했습니다!' : '참여자들이 답변을 작성하고 있습니다...') + '</p></div></div>';
  ...
  async function renderUserView(el, room, myMember, members, headers, roomId, esc) {
    ...
    try {
      ...
      if (!document.getElementById("q7Input")) {
        el.innerHTML = '<div class="fade-in"><div class="text-center mb-6"><div class="text-2xl font-bold">질문에 답해주세요</div><p class="text-gray-400 text-sm">솔직하게 작성해주세요</p></div>'
          + '<div class="card p-5 mb-4"><label class="text-pink-400 font-bold text-sm block mb-2">Q7. ' + esc(QUESTIONS[7]) + '</label><textarea id="q7Input" rows="3" placeholder="답변을 입력해주세요..." class="w-full p-3 rounded-xl bg-black/30 border border-white/10 text-white focus:border-pink-400 focus:outline-none resize-none"></textarea></div>'
          + '<div class="card p-5 mb-6"><label class="text-pink-400 font-bold text-sm block mb-2">Q8. ' + esc(QUESTIONS[8]) + '</label><textarea id="q8Input" rows="3" placeholder="답변을 입력해주세요..." class="w-full p-3 rounded-xl bg-black/30 border border-white/10 text-white focus:border-pink-400 focus:outline-none resize-none"></textarea></div>'
          + '<button id="submitAnswerBtn_g33" onclick="window._submitAnswers_g33()" class="btn-primary w-full text-lg">제출</button></div>';
      }
    } catch(e) { el.innerHTML = '<div class="text-center py-8"><p class="text-red-400">데이터 로드 실패</p></div>'; }
  }

  window._submitAnswers_g33 = async function() {
    var q7 = document.getElementById("q7Input").value.trim();
    var q8 = document.getElementById("q8Input").value.trim();
    if (!q7 || !q8) return alert("두 질문 모두 답변해주세요.");
    var btn = document.getElementById("submitAnswerBtn_g33");
    btn.disabled = true; btn.textContent = "제출 중...";
    try {
      var res = await fetch("/api/gauge/room/" + gaugeUtil.roomId + "/answer", { method: "POST", headers: gaugeUtil.headers, body: JSON.stringify({ answers: [{ question_no: 7, answer_text: q7 }, { question_no: 8, answer_text: q8 }] }) });
