// GAME3_4 단계: 게이지 배분 (닉네임별, Q7/Q8 답변 표시)
(function() {
  var _gaugeState = {};
  var _loaded = false;
  ...
  window.render_GAME3_4 = function(el, room, myMember, members, isAdmin, headers, roomId, stepChanged) {
  ...
  async function renderAdmin(el, room, members, headers, roomId, esc) {
    try {
      var res = await fetch("/api/gauge/room/" + roomId + "/gauge-scores?game_step=GAME3_4", { headers: headers });
      ...
      el.innerHTML = '<div class="fade-in"><div class="text-center mb-4"><div class="text-2xl font-bold">Game 3-4 게이지 배분</div>'
  ...
  async function renderUser(el, room, myMember, members, headers, roomId, esc) {
    ...
    if (!_loaded || !document.getElementById("bubbleContainer_g34")) {
      try {
        var res = await fetch("/api/gauge/room/" + roomId + "/game-answers?question_nos=7,8", { headers: headers });
        ...
        var html = '<div class="fade-in"><div class="text-center mb-4"><div class="text-2xl font-bold mb-1">이성의 답변을 읽고</div><div class="text-lg text-pink-400">게이지를 배분하세요</div>'
          + '<div class="card inline-block px-4 py-2 mt-3"><span class="text-gray-400 text-sm">남은 게이지: </span><span id="remainGauge_g34" class="text-2xl font-black text-pink-400">7</span><span class="text-gray-500 text-sm"> / 7</span></div></div>'
          + '<div id="bubbleContainer_g34" class="space-y-4 mb-6">';
        ...
        Object.keys(persons).forEach(function(memberSeq) {
          ...
          html += '<div class="card p-4"><div class="flex items-center justify-between mb-2"><div class="text-xs text-gray-500">익명 ' + p.person_id + '</div>'
            + '<div class="flex items-center gap-2"><input type="range" min="0" max="7" value="' + gaugeVal + '" class="accent-pink-500 gauge-slider-g34" data-member="' + memberSeq + '" style="width:120px;" />'
            + '<span class="text-lg font-bold text-pink-400 w-6 text-center gauge-val-g34" data-member="' + memberSeq + '">' + gaugeVal + '</span></div></div>';
        ...
        html += '</div><button id="confirmGaugeBtn_g34" onclick="window._confirmGauge_g34()" class="btn-primary w-full text-lg">확인</button></div>';
        el.innerHTML = html;
        _loaded = true;

        document.querySelectorAll(".gauge-slider-g34").forEach(function(slider) {
          slider.addEventListener("input", function() {
            ...
            var valEl = document.querySelector('.gauge-val-g34[data-member="' + memberSeq + '"]');
            if (valEl) valEl.textContent = newVal;
            var totalUsed = Object.values(_gaugeState).reduce(function(s, v) { return s + v; }, 0);
            var remainEl = document.getElementById("remainGauge_g34");
            if (remainEl) remainEl.textContent = 7 - totalUsed;
            document.querySelectorAll(".gauge-slider-g34").forEach(function(other) {
  ...
  window._confirmGauge_g34 = async function() {
    var scores = [];
    ...
    var btn = document.getElementById("confirmGaugeBtn_g34");
    btn.disabled = true; btn.textContent = "제출 중...";
    try {
      var res = await fetch("/api/gauge/room/" + gaugeUtil.roomId + "/gauge-score", { method: "POST", headers: gaugeUtil.headers, body: JSON.stringify({ scores: scores, game_step: "GAME3_4" }) });
