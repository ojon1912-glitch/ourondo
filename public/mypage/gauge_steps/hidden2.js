// HIDDEN2 단계: 히든 게이징 타임 (Round 2)
(function() {
  var _confirmed = false;
  var _userDone = false;

  window.render_HIDDEN2 = function(el, room, myMember, members, isAdmin, headers, roomId, stepChanged) {
    var esc = gaugeUtil.esc;
    if (stepChanged) { _confirmed = false; _userDone = false; }

    if (isAdmin) {
      renderAdminHidden2(el, room, members, headers, roomId, esc);
    } else {
      ...
      renderUserHidden2(el, room, myMember, members, headers, roomId, esc);
    }
  };

  async function renderAdminHidden2(el, room, members, headers, roomId, esc) {
    try {
      var res = await fetch("/api/gauge/room/" + roomId + "/hidden-status?game_step=HIDDEN2", { headers: headers });
      ...
      el.innerHTML = '<div class="fade-in"><div class="text-center mb-4"><div class="text-2xl font-bold">히든 게이징 타임 (Round 2)</div>'
  ...
  async function renderUserHidden2(el, room, myMember, members, headers, roomId, esc) {
    if (!myMember) return;
    ...
    if (!document.getElementById("hiddenContainer_h2")) {
      try {
        ...
        var options = targets.map(function(t) {
          return '<label class="card p-4 flex items-center gap-4 cursor-pointer hover:border-pink-400/50 transition min-h-[56px]"><input type="radio" name="hiddenTarget_h2" value="' + t.member_seq + '" class="accent-pink-500 w-6 h-6 shrink-0" /><span class="text-lg">' + esc(t.nickname || t.name) + '</span></label>';
        }).join("");
        options += '<label class="card p-4 flex items-center gap-4 cursor-pointer hover:border-gray-400/50 transition min-h-[56px]"><input type="radio" name="hiddenTarget_h2" value="none" class="accent-gray-500 w-6 h-6 shrink-0" /><span class="text-lg text-gray-400">없음</span></label>';

        el.innerHTML = '<div class="fade-in" id="hiddenContainer_h2"><div class="text-center mb-6"><div class="text-3xl font-bold glow-text mb-2">히든 게이징 타임! (Round 2)</div>'
          ...
          + '<button id="hiddenConfirmBtn_h2" onclick="window._confirmHidden_h2()" class="btn-primary w-full text-lg">확인</button></div>';
      } catch(e) { el.innerHTML = '<div class="text-center py-8"><p class="text-red-400">데이터 로드 실패</p></div>'; }
    }
  }

  window._confirmHidden_h2 = async function() {
    var selected = document.querySelector('input[name="hiddenTarget_h2"]:checked');
    if (!selected) return alert("한 명을 선택해주세요.");
    var btn = document.getElementById("hiddenConfirmBtn_h2");
    btn.disabled = true; btn.textContent = "제출 중...";
    var toMemberSeq = selected.value === 'none' ? null : parseInt(selected.value);
    try {
      var res = await fetch("/api/gauge/room/" + gaugeUtil.roomId + "/hidden-select", { method: "POST", headers: gaugeUtil.headers, body: JSON.stringify({ to_member_seq: toMemberSeq, game_step: 'HIDDEN2' }) });
